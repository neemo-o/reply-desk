...
export class BotsService {
  ...
  async create(tenantId: string, dto: CreateBotDto) {
    ...
    return this.prisma.bot.create({
      data: {
        ...
        type: dto.type,
        status:
          dto.testContactPhone && dto.type !== "AUTO" ? "testing" : "draft",
  ...
  async update(tenantId: string, id: string, dto: UpdateBotDto) {
    ...
    const wasInactiveForLimits =
      bot.status === "draft" || bot.status === "inactive";
    const willCountAsActive =
      nextStatus === "active" || nextStatus === "testing";
  ...
  async createTrigger(
    tenantId: string,
    botId: string,
    dto: CreateBotTriggerDto,
  ) {
    const bot = await this.assertAgentsBot(tenantId, botId);
    if (
      dto.tipo === "keyword" &&
      (!dto.valor || dto.valor.trim().length === 0)
    ) {
  ...
  async updateTrigger(
    ...
    if (dto.valor !== undefined)
      data.valor = dto.tipo === "first_message" ? null : dto.valor;
  ...
  async createStep(tenantId: string, botId: string, dto: CreateBotStepDto) {
    ...
    if (bot.type === "SIMPLE") {
      if (dto.ordem !== 1) {
        throw new BadRequestException(
          "Bot SIMPLE aceita apenas um step com ordem=1",
        );
      }
      if (!["text", "media"].includes(dto.tipoMensagem)) {
        throw new BadRequestException(
          "Bot SIMPLE aceita apenas step tipo=text ou tipo=media",
        );
      }
    ...
    if (existing)
      throw new BadRequestException(`Já existe step com ordem ${dto.ordem}`);
  ...
  async updateStep(
    ...
    if (bot.type === "SIMPLE") {
      if (
        dto.tipoMensagem !== undefined &&
        !["text", "media"].includes(dto.tipoMensagem)
      ) {
        throw new BadRequestException(
          "Bot SIMPLE aceita apenas step tipo=text ou tipo=media",
        );
    ...
    if (dto.ordem !== undefined) {
      ...
      if (clash)
        throw new BadRequestException(
          `Já existe outro step com ordem ${dto.ordem}`,
        );
    }
    const validated =
    ...
    return this.prisma.botStep.update({
      ...
      data: {
        ...
        ...(dto.condicoesProximo !== undefined
          ...
          : {}),
        ...(dto.fallbackStepOrder !== undefined
          ? { fallbackStepOrder: dto.fallbackStepOrder }
          : {}),
      },
    });
  }
